...
　　在初期编译器不那么智能的时候，变量的类型匹配很大程度上依靠于指定的类型，但随着编译器的发展，为了纠正程序的错误，编译器被给予了推算变量类型的能力。既然编译器可以自己推算出一些类型，那么就代表在一些情况下我们可以不用指定类型而让编译器去推算。模板（templates）就是 C++ 依此而改造并加以设计的新特性。
...
　　注意 Compare 函数使用了比较运算符，而对于自定义的类可能不包含比较运算符的 **重载函数**，此时我们可以 ***重载函数模板*** ：
