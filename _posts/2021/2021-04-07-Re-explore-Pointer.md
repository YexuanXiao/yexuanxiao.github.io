...
　　到了现代，x86 以及 x64，x86_64 等 CPU，不再有寄存器的区分，而是人为规定的名字和按需使用寄存器，指令的最大操作数也不一样。

　　但是在学习 C 语言的时候，我们会发现 32 位程序中的指针（地址）是一个 32 位的二进制数，而 64 位程序中地址是一个 64 位二进制数。

　　这是因为，CPU 中用来用来存储CPU要读取指令的地址的 EIP 寄存器的容量是 32 位或者 64 位（注意，起决定性作用的还是 EIP 寄存器）。

　　 **32 位地址所能表示的内存空间最高是 4GB** 。

　　而有些时候，我们会发现有一部分 CPU 或者操作系统，即使是 32 位的，或者说 EIP 寄存器大小是 32 位，也能支持 4GB 以上的内存。这是由于 CPU 的寻址总线大于 32 位，并且操作系统支持 PAE 技术，软件也可以使用 AWE 技术来访问大于 4GB 的部分。

　　并且，现代的 64 位 CPU 实际上也没有 64 根寻址总线，因为不需要这么多。如果物理内存大于寻址总线数量，那么多余的部分是无论如何也不能访问的（即使用 PAE 或者 AWE）。

　　而在现代操作系统中，大部分软件中通过指针得到的地址实际上不是真正在物理内存上的地址，只有一些硬件驱动可以直接操作物理内存（地址）。
...
　　我们知道，传递指针实际上是传递指针中储存的地址，因此，函数单纯调用指针是不能修改指针指向的地址的，并且如果该指针是一个野指针，还会发生严重的内存错误。
...
int main (void) {
    int *b = (int *)malloc(sizeof(int));//注意，在实际应用中，必须判断malloc函数是否成功申请内存
}
...
　　如果我们直接把 b 传给 change，由于在 main 函数中 b 只经过声明，没有赋值（初始化），所以在执行 change 函数之前，b 是一个野指针，野指针的内容无法控制，代表着我们实际调用是这样的 `change(没人知道是啥，&a)`，此时程序是错误的。
