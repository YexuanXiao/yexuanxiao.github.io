...
　　C++ 默认所有源文件中的函数都是 extern 的，即可被其他编译单元（源文件）使用。我们有时候也可以用 static 关键字使某一函数的作用域是当前文件。
...
　　有时候我们需要使用动态的链接库或者静态的链接库去构建我们的程序，很多情况下你并没有这些库的源码，只有头文件或者头文件和二进制文件，那么你得到或者使用的库的二进制文件很可能是使用 C 编译器编译的。
