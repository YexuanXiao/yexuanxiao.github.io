...
3. PowerShell 执行 wsl 或者直接在开始菜单打开 Ubuntu
...
　　这里我选择安装 gcc-10 和 g++ 10 而不是默认的 gcc-7

1. wsl 执行 `sudo add-apt-repository ppa:ubuntu-toolchain-r/test`
2. wsl 执行 `sudo apt-get update`
3. wsl 执行 `sudo apt install gcc-10`
3. wsl 执行 `sudo apt install g++-10`
3. wsl 执行 `sudo apt install gdb`
	...
	"tasks": [
		{
			...
			"command": "/usr/bin/gcc-10",//改成自己需要用的编译器
			"args": [
				"-g",
				"${file}",
				"-o",
...
　　有时候会遇到 VSCode 提示更新 includePath，这时候需要在 wsl 里执行 `gcc-10 -v -E -x c++ -`

　　此时程序会输出一些信息，我们按下 ctrl+C 终止程序，然后找到这句：`#include <...> search starts here:`

　　直到这句：`End of search list.`

　　我们把中间的内容处理为如下格式并储存为 .vscode 下的文件：
