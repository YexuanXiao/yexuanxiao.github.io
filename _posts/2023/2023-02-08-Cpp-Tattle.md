...
C++ 从早期开始就会在满足条件的情况下隐式生成一些函数以及成员函数，例如默认构造函数，拷贝构造函数，赋值操作符的重载以及析构函数；C++ 11 后有多了两个函数：移动构造函数和赋值操作符对右值的重载；并且可以使用 default 关键字来主动的生成这些函数，用于在这些函数被抑制的时候重新指示编译器生成这些函数，或者单纯是为了清晰性显式声明。
...
此外，编译器生成的拷贝构造，对基本类型成员采用逐成员拷贝，非基本类型则采用对应的拷贝构造。

编译器生成的拷贝赋值，对基本类型成员采用逐成员拷贝，非基本类型则采用对应的拷贝赋值。

编译器生成的移动构造，对基本类型成员采用逐成员拷贝（因为基本类型的移动就是拷贝），非基本类型则采用对应的移动构造（如果有拷贝没有移动则再回退到拷贝）。

编译器生成的移动赋值，对基本类型成员采用逐成员拷贝，非基本类型则采用对应的移动赋值（如果有拷贝没有移动则再回退到拷贝）。

#### 复制消除以及返回值优化

函数的返回值显然要对 caller 可见，因此必定保存在 caller 的栈或者寄存器内，基本类型、引用以及指针都可以直接拷贝，没有额外开销，不在此文范围内；对于自定义类型，当储存在寄存器时则和基本类型一致，当储存在栈时则可能需要考虑是否存在拷贝/移动。
...
此外，如果 caller 使用的是声明及初始化语法，或者在声明后第一次使用时即为赋值，则直接重用 caller 中该变量的储存空间，如果不是，则要考虑作为右值的返回值是的拷贝或者移动。
...
因此，第二个问题产生了：右值引用不是对象。右值引用不是对象，导致绑定右值引用是不触发移动构造和移动赋值的，因此std::move原本设计的意思是转移资源，但现在多了绑定到右值引用的情况，它就不是转移资源了。可以考虑到标准制定人可能觉得对象的移动构造仍然有开销，选用了没开销的方式。
...
这些函数包括拷贝构造函数，拷贝赋值函数，移动构造函数，移动赋值函数以及析构函数，因为这些函数都涉及到资源管理。不包括其他的构造函数。

假设一个类不持有资源，那么他就不需要析构函数，并且拷贝构造函数也不需要申请资源，拷贝赋值函数就不需要释放旧的资源并申请新的资源，移动构造和移动赋值函数就不需要释放旧资源移动新资源，只需要机械的拷贝每一个成员即可，这正是编译器生成的函数所做的事。
