...
`allocate_guard` 这个词可能很多人没听过，不过，实际上它非常常见而且经验经验的人大概率已经独自发明过了。
...
现在设计它的构造函数：理应支持默认构造，移动构造；并且应该避免 `make_shared`，因此它需要支持从 `T&&`，`const T&` 构造，以及支持 `emplace` 构造避免多余的拷贝和移动，得到如下代码：
