...
因此笔者从 0 开始，从理论到实践去实现一个协程接口的线程池，并且模仿 C++/WinRT 风格实现了通过协程去使用线程池，可以说不仅仅是一个协程教程，也是一个异步教程。由于本文和代码皆由笔者一人编写，如有疏漏和错误，还请指正和包涵。同时，本教程为了简化起见，尽量利用标准库已有功能，因此存在一定优化空间，实际应用时可自行优化。

作为前置要求，本文需要读者至少有掌握《C++ Primer 第五版》的水平，同时本文也涉及了《C++ 并发编程实战 第二版》的一部分内容：锁、线程、原子、信号量，如果读者阅读过此书最好。在阅读本文之前，读者也可以准备好 C++ Reference 中相应的页面。许传奇等人的教程可以作为阅读本文前的开胃小菜，但不阅读也没关系。同时，Lewis Baker 的教程仍然是最全面的协程教程，C++ Reference 的协程部分是标准文档的重新编排，是理解协程的微言大义重要的资料。

C++ 的协程是精简且强大的，因此可以说是千人千面，笔者的思路和其他人大概率是不同的，因此注定只能是抛砖引玉，并不能代表协程的全貌。笔者以模仿 C++/WinRT 为出发点，但并不是仿照它，本文只是一个教学。笔者会尽量保证内容的全面和准确，至于其他使用方式，留给其他人以及未来的读者。
...
同时，为了使用线程池，仿照 C++/WinRT 提供以下 API：
...
这些 API 来自 C++/WinRT，读者可以在[使用 C++/WinRT 执行并发和异步操作](https://learn.microsoft.com/zh-cn/windows/uwp/cpp-and-winrt-apis/concurrency)和[通过 C++/WinRT 实现高级并发和异步](https://learn.microsoft.com/zh-cn/windows/uwp/cpp-and-winrt-apis/concurrency-2)找到，但注意我对他们进行了一些修改。