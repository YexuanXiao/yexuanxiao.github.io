...
> 当我们赋给无符号类型一个超出它范围的值时，结果是初始值对无符号类型表示数值总数取模后的余数。<br>例如，8 比特大小的 unsigned char 可以表示 0 至 255 区间的值，如果我们赋了一个区间外的值，比如把 -1 赋给 unsigned char，则实际的结果是 255。

　　我们知道，取模和取余运算过程是相似的，废话不多说，直接上代码：
...
　　由于 C 语言标准中对于许多行为的定义很模糊，所以编译器一般根据 C++ 的标准补充 C 语言的标准，在这个问题上 C 和 C++ 保持一致。