...
categories: [blog]
---
　　Surface Book 3 最重要的功能之一是笔和触摸。我买的 Surface Pen 是第五代的，笔身下部有一个按键，顶部有一个按键。按键本身是使用蓝牙来输入的，不过微软并没有提供宏功能，这时候就需要我们用 VBScript 花式整活。
...
　　那么这些仅仅是模拟具有表面意义的键，并且不支持操作符。那么我们有没有方法进行更骚的操作，比如模拟 `+` ,或者模拟翻页键，前后键，音量键呢?
...
　　我们只需要把拼装出来的 &h88ad （这里将其认为是 GBK 编码 16 进制）输入到第二行，得到一个汉字，比如 &h88ad 代表 埑。
...
　　虽然其本意并不是字符，但是通过这种方式我们依然能得到一样的结果（除非你在时区设置里开启了体验上非常糟糕的 UTF-8 支持）。
