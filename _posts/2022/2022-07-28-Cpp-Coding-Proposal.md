...
35. 避免混用有符号/无符号类型进行算术运算（但允许在得有保证的情况下进行赋值）
    ...
    理由：
    1. C++ 中同样宽度的有符号整数和无符号整数进行算术运算时会提升到同样宽度的无符号数，将导致结果难以理解
    ...
    这是由于 x 被提升为 unsigned 以和 y 进行计算，导致计算的结果是无符号的 4294967294，同时将无符号的 4294967294 赋值给 32 位有符号整数将得到 -2（存在溢出），但赋值给 64 位的有符号整数则得到原值 4294967294。