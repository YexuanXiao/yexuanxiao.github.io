...
categories: [blog]
---
之前的文章文章讲述了 C++ 异常的概念和如何保证基本的异常安全，本文负责讲述如何通过智能指针等技术，设计 Handle 类，保证无内存泄漏以及强异常安全，如何实现安全，异常安全以及高效的构造，拷贝，移动这三大成员函数。
...
由于 `AppleProperty` 的成员都是基本类型，并且没有代表外部资源，那么这个 `AppleProperty` 类就是个 **值类型**：完全 **可以使用编译器生成的构造，赋值，交换和移动**。
...
`String` 类就是一个 Handle 类，因为其成员有一个指针，该指针指向某个不属于 `String` 类自身的外部资源（字符串缓冲区）。
...
一个实现优良的 `String` 类能像值类一样构造，赋值，交换和移动，换句话说 **一个实现优良的 Handle 类能像值类一样构造，赋值，交换和移动**：可以使用 `()` 进行构造，使用 `=` 进行赋值，使用 `std::swap` 进行交换，使用 `std::move` 进行移动。 **Handle 类使得无论多复杂的对象，都和值一样自然的使用**。
...
现在观察上面的 `AppleProperty` 类和 `String` 类，分别对应值类和 Handle 类：
...
3. **Handle 类的默认初始化通常是无意义的** 。例如默认初始化一个 `String`，肯定会初始化为空串，此时就没必要为这个 `String` 分配内存，你也可以尝试输出 `std::vector<int>().capacity()`，即默认构造的 `std::vector` 的预分配空间，结果是 `0`。如果存在一个 `File` 类持有某个文件资源，那么这个 `File` 类的默认构造就是不持有任何文件，而不是持有一个新文件，因为连新文件的名字也没有。再比如一个数据库连接类，总不能凭空获得一个数据库连接。
4. Handle 类的每个成员通常是不独立的，例如 `String`。
...
换句话说，只含有 Handle 类和值类的类，即复合类，其表现也如同值类：**可以使用编译器生成的构造，赋值，交换和移动**。
...
对于上面值类和 `Handle` 类的例子，都是很清晰很自然的情况，但是世界是复杂的，不是每个程序都有良好设计的 `Handle` 类。
...
由于 `std::unique_ptr` 是一个 Handle 类模板，所以 `std::unique_ptr<T>` 表现为值类，构造的时候和值一样构造，使得 `Mixin` 从 Handle 类变为了值类，那么整个构造过程就是异常安全的。
...
#1 处用到了拷贝赋值，但由于并不存在自定义的拷贝赋值，所以编译器会粗暴的执行 `a.intptr = b.intptr`，那么 a 就存在内存泄漏和 double free（对 `b.intptr` 的值 `delete` 两次），程序会因此崩溃。
...
在上面的内容中，说明了第一点原因：Handle 类的默认构造没意义，因为你构造不出默认的文件，默认的数据库连接。在此，给出第二点原因： **和容器兼容**。
...
其实文章看到这里，才真正看到写文章时最初的目的。直接构造，拷贝赋值，拷贝构造，实际上都是非常简单的内容，没有任何难度，而下面所述就相对复杂了，但经过分析还是简单的。
