...
QueryResult TextQuery::query(const string& expect) const {
	...
	auto loc = result.find(expect); // 查找关键字，返回迭代器
...
ostream& operator<<(ostream& o, const QueryResult& qr) {
	o << qr.expect << " occors " << qr.lines->size() << " times " << endl; // 关键字和出现次数
	for (auto num : *qr.lines)
	{
		o << " \t(line " << num + 1 << ")" << *(qr.text->begin() + num) << endl; // 关键字所在行
	}
	return o;
}
